...
def get_product_info(site_name, product):
	logger = logging.getLogger()
...
def download_images(site_name, product, base_path):
	logger = logging.getLogger()
	paths = None
...
class ImageDownloader:
	...
	def run(self):
		...
		worker_logger_name = "{}{}.log".format(base_worker_logger_name, self.site_name)

		init_logger(worker_logger_name)
		...
		try:
			...
			products, xml_timestamp = get_products(self.site_name)
			if products is None:
				logger.error("Skipping site {} due to error while getting product list".format(self.site_name))
				return

			for product in products:
				paths = download_images(self.site_name, product, self.base_path)
				if paths:
				...
				product_info = get_product_info(self.site_name, product)
				if product_info:
